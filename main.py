...
def connect():
    if "Api-Key" not in request.headers or request.headers.get('Api-Key') != os.getenv('API_KEY'):
        ...
        disconnect()
        return
    socketio.emit("info", {"message":"TESTANDOOOOOOOOOOOOOOOOOOOO."})
...
def process(json):
    try:
        p = psutil.Process(json["pid"])
        p.terminate()
        socketio.emit("info", {"message":f"Processo {p.name()}({p.pid}) finalizado."})
    except:
        socketio.emit("info", {"message":"PID não existe."})
